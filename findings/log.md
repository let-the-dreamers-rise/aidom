...
| 2026-09-13 | StackingDAO | on-chain Clarity, 36 listed + post-scope versions, PoX-5 stBTC launch (Aug 2026) | 2 verified: (1) stBTC gets 0.004% of the sBTC reward stream, High-tier argument, ~USD 21.6k short to date; (2) ungated save-pending-rewards bricks claims on deactivated positions, Low today | 1 (report 01 clears all four gates) | submitted 2026-09-13 by direct private disclosure (email to team, PDF + verifier attached); Immunefi form demanded 50 USDC. Report 02 FINALIZED 2026-09-14 (PDF + read-only verifier, live-verified: v3/v4/v5 D<G reachable, v6 active); ready to send alongside report 01. Follow-up: DM @StackingDAO on X if no reply by 2026-09-18 | pending |
...
| 2026-09-14 | Hermetica (Immunefi $100k, Clarity) | hermetica-contracts @5e13e43, deployed code cross-checked | mint/redeem unbacked; staking inflation; vault double-spend/cancel-after-fund; silo double-withdraw | 1 (staking inflation, Low/conditional) | drafting (Low) | mint/redeem operator-gated; vault uses internal accounting (safe); silo safe; staking reads donatable reserve balance w/ no dead shares — not exploitable at 1.06M supply |
| 2026-09-14 | Velar univ2-core (Clarity AMM) | on-chain SP1Y5YSTAHZ...univ2-core/router/library | swap k-invariant bypass; mint inflation; burn over-withdraw; ungated create | 0 | nothing_found | create owner-gated; swap enforces fee-adjusted a*b>=k with full revert; reserves are internal state (no donation/skim); burn proportional rounds to pool; collect fee-to-gated. (cosmetic: swap fee-precondition parens quirk, not fund-loss) |
| 2026-09-14 | Charisma / Blaze (Clarity, deep dive) | on-chain blaze-v1 + multihop; repo r0zar/charisma x-* | blaze verifier soundness; subnet transfer binding; x-pool signed-swap recipient; uuid replay | 1 Low (deployed) + 1 pre-deploy warning | drafting(Low)/informational | blaze-v1 sound (contract-bound hash, uuid single-use, LTE has <=bound assert). LOW: global uuid map lets anyone pre-burn a victim's uuid (griefing DoS, no theft). x-pool.x-swap output recipient is unbound (theft) BUT x-pool/x-vault/x-*-token are NOT deployed (404) -> dev code, pre-deployment warning only. multihop (deployed) credits tx-sender, no bug. |
| 2026-09-14 | Arkadiko vaults-v2 (Clarity, USDA vaults) | arkadiko-dao repo @62095e8, deployer SP2C2YFP... | mint beyond collateral; withdraw undercollateralized; liquidate healthy vault; stability-pool reward theft/rounding | 0 | nothing_found | open/update enforce ratio>=liquidation & canonical-contract checks; liquidate only when (not valid); pool-liq reward snapshots correct, burn-usda gated + MIN_POOL_USDA floor; only dust rounding (actively fixed in #619) |
| 2026-09-14 | StackingDAO "go all in" deep pass | on-chain swap-ststx-ststxbtc-v5, stx-reserve-v2, ststxbtc-pot-rebalancer-v1, reward-split-ops-v2, strategy-v6, reward-split-calculator/adapter-v2, commission-sbtc-v1, stbtc-reserve | forward-swap under-backing (mint value-v stSTXbtc, lock min(value-v,available) STX) → insolvency; keeper sweep/refresh-split abuse | 0 | nothing_found | forward swap conservative: value-v≤ststx·r_real (floor rate), obligation just transfers stSTX→stSTXbtc; min-u lock is correct (never earmark reserve STX not physically free — remainder rides shared staking pool, backing = earmark−idle computed dynamically in return-stx-from-staking-split); round-trip r_floor/r_up≤1 no arb; all reserve mutators check-is-protocol gated; sweep/refresh-split keeper-gated. Reachable external surface exhausted — 2 findings stand (reports 01+02). |
