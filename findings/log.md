...
| 2026-09-14 | Mattermost + Playbooks (HackerOne) | mattermost-src 11.x master; playbooks-src @1d70f61 | missing-authz siblings of 2026 CVEs (recap/agents/access-control/properties; playbook checklist mutations; playbook GraphQL read) | 0 | nothing_found | Layered authz throughout: core api4 fresh features guarded (recap validates channel read-perm, properties hasTargetAccess switch is complete); playbook checklist cluster gated by checkEditPermissions middleware (RunManageProperties); playbook property-fields check PlaybookEdit; GraphQL Playbook(id) checks PlaybookView via getGraphqlPlaybook. Hardened. |
| 2026-09-14 | Documenso (self-hosted) | documenso-src 2.18.0 HEAD | file/PDF routes ignore document visibility (checkEnvelopeFileAccess = team-membership only, no TEAM_DOCUMENT_VISIBILITY_MAP) — sibling/incomplete-fix of CVE-2026-85697 | 1 real code gap, but HIGH duplicate-overlap with CVE-2026-85697 | nothing_submittable | CVE-2026-85697 desc already covers "restricted documents WITHIN their team"; 2.18 fix scoped cross-team but left intra-team visibility bypass. Real at HEAD but likely-duplicate; needs human to compare vs the fix commit before any report. |
